...
namespace CallCenter_Dataspecs.SMSMarketing
{
    namespace Models
    {
        ...
        public interface IFile_QueuedMarketingSMS : IFile_PortalSMS
        {
            string SaccoName { get; set; }
            string ActionUser { get; set; }
            DateTime DateQueued { get; set; }
        }
        ...
        public interface IMSACCO_SMSUnitsPreload
        {
            ISACCO Client { get; }
            int NumSMSUnits { get; set; }
            string ActionUser { get; set; }
            string TrxDescription { get; set; }
        ...
        public class PortalSMSFile : IFile_PortalSMS
        {
            ...
            private int ParseSMSPartsInMessageBody()
            {
                ...
                if (MessageBody.Length <= Utilities.Cellular.GSM_03_38.SMS.SINGLE_MSG_MAX_CHAR)
                {
                    return 1;
                }

                int numParts = Math.DivRem(MessageBody.Length, Utilities.Cellular.GSM_03_38.SMS.MULTIPART_MSG_MAX_CHAR, out int remainder);
        ...
        public class MarketingSMSFile : PortalSMSFile, IFile_QueuedMarketingSMS
        {
            public string SaccoName { get; set; }
            public string ActionUser { get; set; }
            public DateTime DateQueued { get; set; }

            public MarketingSMSFile(List<ISMSRecipient> smsRecipients) : base(smsRecipients)
            {
            }
        }
        public class MSACCOSMSPreload : IMSACCO_SMSUnitsPreload
        {
            public ISACCO Client { get; }
            public int NumSMSUnits { get; set; }
            public string ActionUser { get; set; }
            public string TrxDescription { get; set; }
            public DateTime DateLoaded { get; set; }

            public MSACCOSMSPreload(ISACCO msaccoClient)
            {
                Client = msaccoClient;
            }
        }
    ...
    namespace Functions
    {
        public interface IBL_SMSMarketing
        {
            event EventHandler<Models.IFile_QueuedMarketingSMS> OnSMSQueued;
            event EventHandler<Models.IMSACCO_SMSUnitsPreload> OnSMSUnitsPreloaded;
